...
<tr>
	...
	<font size="+2" color="#3F3FFF3F">Fichiers favoris</font>
	</td>
	<td width="4%"></td>
</tr>
<tr>
	<td width="4%"></td>
	<td width="92%" height="450" bgcolor="#FFFFFF30" valign="top">
		<table cellspacing="0" cellpadding="0">
	        <xsl:for-each select="Favorites/MRL">
		<tr>
		<td width="22%" valign="top" align="center">
	        	<a>
	        		<xsl:attribute name="href">play.html?action=add&amp;_file=<xsl:call-template name="do-escape-uri">
	 					...
	 					</xsl:call-template>&amp;param=Lecture+du+<xsl:value-of select="@kind"/>...</xsl:attribute>
				Jouer
			</a>/
	        	<a>
	        		<xsl:attribute name="href">$explore?_file=<xsl:call-template name="do-escape-uri">
	   					<xsl:with-param name="str" select="."/>
	 					</xsl:call-template>&amp;playAll=focused</xsl:attribute>
				Explorer
			</a>
			<xsl:text disable-output-escaping="yes"><![CDATA[&nbsp;]]></xsl:text>
	        	<a>
	        		<xsl:attribute name="href">favremoved.html?action=favdel&amp;file=<xsl:call-template name="do-escape-uri">
	   					<xsl:with-param name="str" select="."/>
	 					</xsl:call-template>&amp;param=Lecture+du+<xsl:value-of select="@kind"/>...</xsl:attribute>
				Retirer
			</a>
